...
func main() {
	...
	go func() {
		for {
			if i {
				log.Println("Cycle")
				i = false
				if jsonPayload.RepostMakabaEnabled == true {
					repost2ch()
					jsonPayload = JsonPayload{}
				} else {
					log.Printf("Reposting to Makaba is disabled: %v", jsonPayload.RepostMakabaEnabled)
				}
			}
	...
	go func() {
		log.Printf("Makaba consumer ready, PID: %d", os.Getpid())
		for d := range msgs {
